...
function handleLeave($conn) {
    if (isset($_POST['inputAnnual'], $_POST['inputSick'], $_POST['inputHospitalization'])) {
        $annualLeave = mysqli_real_escape_string($conn, $_POST['inputAnnual']);
        $sickLeave = mysqli_real_escape_string($conn, $_POST['inputSick']);
        $hospitalizationLeave = mysqli_real_escape_string($conn, $_POST['inputHospitalization']);
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            echo "<script>alert('Error inserting leave info: " . mysqli_error($conn) . "');</script>";
        }
    } else {
