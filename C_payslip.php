...
// Check if user is authorized (Employer only)
...
$payslip_data = null;
$selected_employee = isset($_POST['employee_id']) ? (int)$_POST['employee_id'] : '';
$year = isset($_POST['year']) ? (int)$_POST['year'] : date('Y');
$no_data_found = false;

// Handle PDF download for specific employee, month, and year
if (isset($_GET['download']) && $_GET['download'] == 'pdf') {
    $download_employee = (int)$_GET['employee_id'];
    ...
    mysqli_stmt_bind_param($stmt, "iii", $download_employee, $download_month, $download_year);
...
// Fetch ALL users (both employees and employers) for dropdown
$employees_query = "SELECT el.ID, el.username, el.role, pi.full_name 
                   FROM employeelogin el 
                   LEFT JOIN personal_information pi ON el.ID = pi.personal_id 
                   ORDER BY el.role DESC, COALESCE(pi.full_name, el.username)";
...
if ($selected_employee && $year) {
    ...
    $stmt = mysqli_prepare($conn, $query);
    mysqli_stmt_bind_param($stmt, "ii", $selected_employee, $year);
    mysqli_stmt_execute($stmt);
    $result = mysqli_stmt_get_result($stmt);
    ...
    <section class="section">
      <div class="row">
        <div class="col-lg-12">
          ...
          <div class="card">
            <div class="card-body">
              <h5 class="card-title">View Employee & Employer Payslips</h5>

              <!-- Filter Form -->
              <form method="POST" class="row g-3 align-items-end mb-4">
                <div class="col-md-4">
                  <label for="employee_id" class="form-label">Select Employee/Employer</label>
                  <select name="employee_id" id="employee_id" class="form-select" required>
                    <option value="">-- Select Employee/Employer --</option>
                    
                    <!-- Employers Section -->
                    <optgroup label="👨‍💼 Employers">
                      <?php foreach ($employees as $employee): ?>
                        <?php if ($employee['role'] === 'Employer'): ?>
                          <option value="<?php echo $employee['ID']; ?>" 
                                  <?php echo ($employee['ID'] == $selected_employee) ? 'selected' : ''; ?>>
                ...
                <div class="col-md-3">
                  <button type="submit" class="btn btn-primary">
                    <i class="bi bi-search"></i> View Payslips
                  </button>
                </div>
                <?php if ($selected_employee): ?>
                <div class="col-md-2">
                  <div class="alert alert-info mb-0 py-2">
                    <small><i class="bi bi-info-circle"></i> Showing <?php echo $year; ?></small>
                  </div>
                </div>
                <?php endif; ?>
              </form>
              ...
              <?php if ($payslip_data): ?>
                <?php 
                // Get employee name and role for display
                $employee_display_name = $payslip_data[0]['full_name'] ?? $payslip_data[0]['employee_name'] ?? 'Unknown User';
                $employee_role = $payslip_data[0]['employee_role'] ?? 'Unknown';
                $role_icon = ($employee_role === 'Employer') ? '🏢' : '👤';
                $role_label = ($employee_role === 'Employer') ? 'Employer' : 'Employee';
                ?>
                
                <div class="alert alert-success">
                  <i class="bi bi-person-check"></i> Displaying payslips for <strong><?php echo $role_icon; ?> <?php echo htmlspecialchars($employee_display_name); ?></strong> 
                  <span class="badge bg-primary ms-2"><?php echo $role_label; ?></span> (Year: <?php echo $year; ?>)
                </div>
                        ...
                        <tr>
                          ...
                          <td>
                            <?php if (strtolower($payroll['status']) === 'confirmed'): ?>
                              <a href="?download=pdf&employee_id=<?php echo $selected_employee; ?>&month=<?php echo $payroll['payroll_month']; ?>&year=<?php echo $payroll['payroll_year']; ?>" 
                ...
                <div class="row mt-4">
                  <div class="col-md-12">
                    <div class="card bg-light">
                      <div class="card-body">
                        <h6 class="card-title">
                          <?php echo $role_icon; ?> <?php echo htmlspecialchars($employee_display_name); ?> 
                          <span class="badge bg-<?php echo ($employee_role === 'Employer') ? 'primary' : 'success'; ?> ms-2"><?php echo $role_label; ?></span> 
                          - Year <?php echo $year; ?> Summary
                        </h6>
                        <div class="row">
                          <div class="col-md-3">
              ...
              <?php elseif ($no_data_found && $selected_employee): ?>
                <div class="alert alert-warning">
                  <i class="bi bi-exclamation-triangle"></i> No payroll data found for the selected user in year <?php echo $year; ?>.
                </div>
              <?php elseif ($_SERVER['REQUEST_METHOD'] == 'POST'): ?>
                <div class="alert alert-info">
                  <i class="bi bi-info-circle"></i> Please select a user to view their payslips.
                </div>
              <?php else: ?>
                <div class="alert alert-info">
                  <i class="bi bi-info-circle"></i> Please select an employee or employer and year to view payslips.
                </div>
              <?php endif; ?>
